...
import com.kodilla.pattern2.facade.ShopService;
    ...
    public void processOrder(final OrderDto order, final Long userId) throws OrderProcessingException{
            ...
            try {
                for (ItemDto orderItem : order.getItems()) {
                    LOGGER.info("Adding item: " + orderItem.getProductId() + ", " + orderItem.getGuantity() + " pcs");
                    shopService.addItem(orderId, orderItem.getProductId(), orderItem.getGuantity());
                }
                ...
                if (!shopService.submitOrder(orderId)){
                    LOGGER.error(OrderProcessingException.ERR_SUBITTING_ERROR);
                    wasError = true;
                    throw new OrderProcessingException(OrderProcessingException.ERR_SUBITTING_ERROR);
                }
